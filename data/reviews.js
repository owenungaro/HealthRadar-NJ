...
export async function createReview(review, userId, hospitalId) {
    ...
    if(sanitizedReview.length < 5 || sanitizedReview.length > 500) throw "Review must be between 5 and 500 characters!";
...
export async function getReviewsByUser(userId) {
    ...
    const desiredReviews = await reviews.find({userID: new ObjectId(userId)}).toArray();

    if(desiredReviews.length === 0) throw "User is either invalid, or there were no reviews with this given user.";
